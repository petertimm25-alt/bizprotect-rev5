...
export function writeDeviceId(id, opts) {
    try {
...
export function readCookieDeviceId() {
    try {
 ...
 */
export function getOrCreateDeviceId(opts) {
    // 1) ลองอ่านจาก localStorage ก่อน
    const existing = readDeviceId();
    if (existing) {
        // sync cookie ถ้าขอ
        if (opts?.alsoCookie)
            writeDeviceId(existing, { alsoCookie: true, cookieDays: opts.cookieDays });
        return existing;
    }
