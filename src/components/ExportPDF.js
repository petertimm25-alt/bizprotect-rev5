...
export default function ExportPDF({ state }) {
    ...
    const openInNewTabIOS = async () => {
        ...
        try {
            const blob = await pdf(_jsx(ProposalPDF, { state: state, plan: (plan ?? 'free') })).toBlob();
    ...
    const onPrimaryClick = (e) => {
        ...
        if (!canExport) {
            try {
                toast('ฟีเจอร์ Export PDF ใช้ได้ในแผน PRO/ULTRA กรุณาอัปเกรด');
            }
            catch { }
            navigate('/pricing');
            return;
    ...
    return (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", onClick: onPrimaryClick, className: "rounded-xl px-4 py-2 md:h-12 bg-[var(--brand-accent)] text-[#0B1B2B] font-semibold hover:brightness-95", title: "Export PDF", children: "Export PDF" }), open && canExport && !isIOS && (_jsx("div", { className: "fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4", children: _jsxs("div", { className: "w-full max-w-5xl rounded-xl bg-[color:var(--page)] ring-1 ring-white/10 overflow-hidden", children: [_jsxs("div", { className: "flex items-center justify-between px-4 py-2 border-b border-white/10", children: [_jsx("div", { className: "text-sm text-[color:var(--ink-dim)]", children: "\u0E15\u0E31\u0E27\u0E2D\u0E22\u0E48\u0E32\u0E07\u0E40\u0E2D\u0E01\u0E2A\u0E32\u0E23 (Preview)" }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { type: "button", onClick: downloadNow, disabled: busy, className: "text-xs px-3 py-1 rounded ring-1 ring-gold/50 hover:bg-gold/10 disabled:opacity-50", children: busy ? 'กำลังสร้าง…' : 'ดาวน์โหลด' }), _jsx("button", { type: "button", onClick: () => setOpen(false), className: "text-xs px-3 py-1 rounded ring-1 ring-white/20 hover:bg-white/10", children: "\u0E1B\u0E34\u0E14" })] })] }), _jsx("div", { className: "h-[80vh] bg-black/10", children: _jsx(BlobProvider, { document: _jsx(ProposalPDF, { state: state, plan: (plan ?? 'free') }), children: ({ url, loading, error }) => {
                                    if (loading)
