...
type Props = {
  ...
  combinedCost: number
  /** ช่องวางคอนเทนต์ด้านขวา (เช่น ปุ่ม Export) */
...
export default function StickySummary({
  ...
  return (
    <div className="sticky top-0 z-40 -mx-6 px-6 py-3 bg-[color:var(--page)]/85 backdrop-blur supports-[backdrop-filter]:bg-[color:var(--page)]/60 border-b border-white/10">
      <div className="max-w-6xl mx-auto flex flex-wrap items-center gap-3">
        {/* ด้านซ้าย: คุมปีภาษี + ปุ่มทางลัด */}
        <div className="flex flex-1 flex-wrap items-center gap-2 min-w-0">
          ...
          <div className="ml-2 flex items-center gap-2">
            <button type="button" onClick={() => go('company-sec')}
              className="bp-btn bp-btn--sm bp-btn--ghost hover:text-[#EBDCA6]">ไปข้อมูลบริษัท</button>
            <button type="button" onClick={() => go('directors-sec')}
              className="bp-btn bp-btn--sm bp-btn--ghost hover:text-[#EBDCA6]">ไปผู้บริหาร</button>
            <button type="button" onClick={() => go('cit-table-sec')}
              className="bp-btn bp-btn--sm bp-btn--ghost hover:text-[#EBDCA6]">ไปตาราง ภ.ง.ด.50</button>
            <button type="button" onClick={() => go('return-sec')}
              className="bp-btn bp-btn--sm bp-btn--ghost hover:text-[#EBDCA6]">ไปสิทธิประโยชน์</button>
          </div>
        </div>

        {/* ด้านขวา: ปุ่ม/แอ็กชัน */}
        <div className="ml-auto flex items-center">{rightSlot}</div>

        {/* สรุปตัวเลข (ขึ้นบรรทัดใหม่) */}
